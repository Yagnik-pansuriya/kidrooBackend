...
/**
 * Automatically regenerates sitemap.xml on disk in the background.
 */
export const triggerSitemapRegeneration = async (): Promise<void> => {
  try {
    const xml = await generateSitemapXml();
    const possiblePaths = [
      path.join(__dirname, "../../../frontend/dist/sitemap.xml"),
      path.join(__dirname, "../../../frontend/public/sitemap.xml"),
      "/var/www/kidrooToys/frontend/dist/sitemap.xml",
      "/var/www/kidrooToys/frontend/public/sitemap.xml",
      "/var/www/kidroo/frontend/dist/sitemap.xml",
      "/var/www/kidroo/frontend/public/sitemap.xml",
    ];
    ...
    for (const targetPath of possiblePaths) {
      try {
        ...
        if (fs.existsSync(dir)) {
          ...
          console.log(`[AutoSitemap] Updated sitemap.xml at ${targetPath}`);
        }
      } catch (err) {
    ...
    console.error("[AutoSitemap] Error updating sitemap.xml:", err);
  }
};
